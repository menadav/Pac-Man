...
class HighScoreScene(BaseScene):
    """High score scoreboard scene presenting the leaderboard rankings."""
    def __init__(self, screen: pygame.Surface) -> None:
        """Initialize the highscore scene container with font assets and options."""
        super().__init__(screen)
        self.index = 0
        self.option = ["Play", "Back to menu"]
        self.font = pygame.font.Font(None, 100)
        self.score_list: List[str] = []

    def high_load(self, data: Optional[List[HighScoreEntry]]) -> None:
        """Format, sort, and slice high scores data down to the top 10 entries."""
        if not data:
    ...
    def handle_events(
        ...
        """Process menu cycling and index validations for the scoreboard screen."""
        for event in events:
    ...
    def select_options(self) -> Tuple[str, int]:
        """Convert selected user selection index into operational state machine tokens."""
    ...
    def draw(self, screen: pygame.Surface) -> None:
        """Render the leaderboard header, active navigation paths, and top records."""
        screen.fill((0, 0, 0))
        ...
        if not self.score_list:
            ...
            for j, line in enumerate(self.score_list):
                ...
                rect = score_surf.get_rect(
                    center=(center_x, scores_base_y + j * scores_spacing)
                )
                if rect.bottom < sh:
